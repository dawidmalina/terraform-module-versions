...
func init() {
	...
	flag.BoolVar(&config.Exit, "e", config.Pretty, "return exit code when updates found (in conduction with -updates)")
	flag.BoolVar(&config.Exit, "exit", config.Pretty, "(alias for -exit)")
